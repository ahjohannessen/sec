...
class StreamsWithRetrySpec extends CatsEffectSuite {
  ...
  test("maxAttempts") {
    ...
    IO.defer {
      ...
      result.compile.toList.attempt.map { att =>
        att shouldEqual RetryErr("5").asLeft
        failures shouldEqual 5
      }
    }
  }
  ...
  test("retryOn") {
    ...
    IO.defer {
      ...
      result.compile.toList.attempt.map { att =>
        ...
        att shouldEqual RetryErr("fatal").asLeft
  ...
  test("delay / maxDelay / backoffFactor") {
    ...
    IO.defer {
      ...
      result.compile.lastOrError.attempt.map { att =>
        getDelays shouldEqual List[Long](1, 2, 4, 8, 8)
        att shouldEqual RetryErr().asLeft
      }
    }
  }
  ...
  test("retryEnabled") {
    ...
    result.compile.toList.attempt.map(_ shouldEqual RetryErr("successes: 0").asLeft)
  }
  ...
  test("logs") {
    ...
    IO.defer {
      ...
      stream.compile.toList.attempt.map(_ shouldEqual RetryErr("OhNoes").asLeft)
  ...
  test("only emit elements that can pass through") {
