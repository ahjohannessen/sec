...
private[sec] object convertV2:
  ...
  private def fromDetail(a: PbAny): Option[EsException] =
    unpackAs[v2e.AppendConsistencyViolationErrorDetails](a).map { d =>
      v2.AppendConsistencyViolation(d.violations.toList.flatMap { cv =>
        cv.`type`.streamState.map { ss =>
          v2.AppendConsistencyViolation.StreamStateViolation(ss.stream, ss.expectedState, ss.actualState)
        }
      })
    } orElse
      unpackAs[v2e.StreamRevisionConflictErrorDetails](a).map { d =>
        v2.StreamRevisionConflict(d.stream, d.expectedRevision, d.actualRevision)
      } orElse
      unpackAs[v2e.AppendRecordSizeExceededErrorDetails](a).map { d =>
        v2.AppendRecordSizeExceeded(d.stream, d.recordId, d.size, d.maxSize)
      } orElse
      unpackAs[v2e.AppendTransactionSizeExceededErrorDetails](a).map { d =>
        v2.AppendTransactionSizeExceeded(d.size, d.maxSize)
      } orElse
      unpackAs[v2e.StreamAlreadyExistsErrorDetails](a).map(d => v2.StreamAlreadyExists(d.stream)) orElse
      unpackAs[v2e.StreamTombstonedErrorDetails](a).map(d => v2.StreamTombstoned(d.stream)) orElse
