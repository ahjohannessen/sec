...
final class MetaStreamsOps[F[_]: ErrorM](val ms: MetaStreams[F]) {
  ...
  /**
   * Sets max count in [[Int]] for a stream and returns [[WriteResult]] with current positions of the stream
   * after a successful operation. Failure to fulfill the expected state is manifested by raising
   * [[sec.api.exceptions.WrongExpectedState]].
   *
   * @param id the id of the stream.
   * @param expectedState the state that the stream is expected to in. See [[StreamState]] for details.
   * @param count the max count [[Int]] value for data in the stream. Valid values are greater or equal
   *              to 1. An [[InvalidInput]] exception is raised for invalid input value.
   */
  ...
  /**
   * Sets cache control in [[FiniteDuration]] for a stream and returns [[WriteResult]] with current positions of the stream
   * after a successful operation. Failure to fulfill the expected state is manifested by raising
   * [[sec.api.exceptions.WrongExpectedState]].
   *
   * @param id the id of the stream.
   * @param expectedState the state that the stream is expected to in. See [[StreamState]] for details.
   * @param cacheControl the cache control [[FiniteDuration]] value for data in the stream.
   *                     Valid values are [[FiniteDuration]] greater or equal to 1 second.
   ...
   * @param id the id of the stream.
   * @param expectedState the state that the stream is expected to in. See [[StreamState]] for details.
   * @param truncatedBefore the truncated before [[Long]] value for data in the stream.
   *                     Valid values are [[Long]] greater or equal to 0L.
   *                     An [[InvalidInput]] exception is raised for invalid input value.
   */
