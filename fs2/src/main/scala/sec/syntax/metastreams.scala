...
trait MetaStreamsSyntax:
  ...
  extension [F[_]: MonadThrow](ms: MetaStreams[F])
    ...
    /** Sets max age in [[scala.concurrent.duration.FiniteDuration]] for a stream and returns [[sec.api.WriteResult]]
      ...
      * manifested by raising [[sec.api.exceptions.WrongExpectedState]].
      *
      * @param id
    ...
    /** Sets cache control in [[scala.concurrent.duration.FiniteDuration]] for a stream and returns
      * [[sec.api.WriteResult]] with current positions of the stream after a successful operation. Failure to fulfill
      * the expected state is manifested by raising [[sec.api.exceptions.WrongExpectedState]].
      *
      * @param id
      *   the id of the stream.
      * @param expectedState
